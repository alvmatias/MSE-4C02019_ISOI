...
/**
* @var Configuracion GPIO y SCU Pin de cada indice de GPIO.
* @note const
*/
...
static IRQn_Type IRQ_Type[MAX_GPIO_CHANNEL] = 
{
   PIN_INT0_IRQn
,  PIN_INT1_IRQn
,  PIN_INT2_IRQn
...
/**
 * @brief   Obtencion de la Configuracion del GPIO
 * @param   pin            : Indice del GPIO a obtener la configuracion.
 * @param   pinNamePort    : Puntero al SCU Port del GPIO
 * @param   pinNamePin     : Puntero al SCU Pin del GPIO
 * @param   func           : Puntero al numero de funcion del GPIO
 * @param   gpioPort       : Puntero al GPIO Port
 * @param   gpioPIn        : Puntero al GPIO Pin
 * @return  Nada
...
{
   int8_t pinNamePort = 0; /** pinNamePort : SCU Port del GPIO */
   int8_t pinNamePin  = 0; /** pinNamePin : SCU Pin del GPIO */

   int8_t func        = 0; /** func : numero de funcion del GPIO */

   int8_t gpioPort    = 0; /** gpioPort : GPIO Port */
   int8_t gpioPin     = 0; /** gpioPIn : GPIO Pin */
...
void gpioConfigIRQ(gpioMap_t pin, uint8_t channel, edgeInt_t edgeInt)
{
   ...
   *                      |          |             |    */
   ...
   NVIC_ClearPendingIRQ(IRQ_Type[channel]);
...
uint8_t gpioRead(gpioMap_t pin)
{
   ...
   return Chip_GPIO_ReadPortBit(LPC_GPIO_PORT, gpioPort, gpioPin);
}
